...
from .segment_anything import build_aff_openad,build_aff_openad_output,build_aff_openad_output_train
...
@registry.register_model("aff_vicuna_openad")
class AffordanceVicunaOpenAD(BaseModel):
    ...
    def __init__(
        ...
        self.seg_point_encoder = build_model_checkpoint(openadcfg,seg_point_encoder_path,is_eval=True)    
...
# pred mask
    def predict_mask(
        ...
        projected_hidden_state = self.aff_proj(last_hidden_states)  # [bs, length, dim],2*40*512
        ...
        for i in range(projected_hidden_state.shape[0]):
            ...
            if seg_token_mask.sum() == 0:
                pred_masks.append(torch.zeros((1, *original_list), dtype=torch.float32).cuda())
                continue

            seq_length = last_hidden_states.shape[1]
            seg_token_mask = seg_token_mask[:seq_length]

            pred_embeddings_ = projected_hidden_state[i][seg_token_mask]  # [num_seg, dim],示例40*512-->1*512,只有1个seg
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast(self.mix_precision):
            if "points" in samples:
                ...
                inputs_llm, atts_llm, point_pos_em = self.encode_point(points)#此时atts_llm为b*513
            ...
            output_ids = torch.concat(
                [output_ids, torch.full((output_ids.shape[0], 1), pad_token_id).to(torch.long).cuda()],
                dim=1
            )
        ...
        # Debugging code
        if any(math.isnan(loss.item()) for loss in (ce_loss, mask_bce_loss, mask_dice_loss, total_loss)):
    ...
    def generate(
        ...
        with self.maybe_autocast(self.mix_precision):           
            ...
            if "points" in samples:
                ...
                last_hidden_states = [token_states[-1] for token_states in outputs.hidden_states[1:]]   # remember to drop input token
...
# It is necessary to set different parameters with different learning rates.
    ...
    def from_config(cls, cfg): 
        ...
        # Point_encoder set
        point_model_config_path= cfg.get("point_model_config_path", '/workspace/project/Aff_LLM_debug/configs/models/PointTransformer_2048point.yaml')
        freeze_point= cfg.get("freeze_point", True)
        # seg_encoder 
        free_seg_point_encoder=cfg.get("free_seg_point_encoder",False)
        seg_point_encoder_config_path=cfg.get("seg_point_encoder_config_path",'/workspace/project/Aff_LLM_debug/models/openad/config/full_shape_cfg_DGCNN_modify.py')
        seg_point_encoder_path=cfg.get("seg_point_encoder_path","/workspace/project/Aff_LLM_debug/model_ckpt/DGCNN_best_model.t7")
        #aff_decoder
