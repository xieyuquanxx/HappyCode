...
class EquipAction(ItemListAction):
    ...
    def __init__(self, items: list, _default="none", _other="other"):
        ...
        (
            super().__init__(
                self._command, self._items, _default=_default, _other=_other
            ),
        )
    ...
    def from_universal(self, obs):
        try:
            if (
                ...
                item = self._univ_items.index(
                    obs["slots"]["gui"]["slots"][-10 + hotbar_index]["name"]
                )
                if item != self.previous:
                    self.previous = item
                    return obs["slots"]["gui"]["slots"][-10 + hotbar_index][
                        "name"
                    ].split("minecraft:")[-1]
        except KeyError:
